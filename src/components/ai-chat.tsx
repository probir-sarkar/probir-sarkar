...
export default function AiChat() {
  ...
  const verificationFailed = chat.error?.message.includes('403') ?? false

  useEffect(() => {
    if (verificationFailed) setVerified(false)
  }, [verificationFailed])

  const verify = async (turnstileToken: string) => {
          ...
          >
            ...
            {!verified ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-4 p-6 text-center">
                {siteKey && (
                  <Turnstile
                    onSuccess={(token) => void verify(token)}
                    options={{
                      theme: 'dark',
                      size: 'normal',
                      action: 'chat',
